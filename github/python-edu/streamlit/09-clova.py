...
class CompletionExecutor:
    ...
    def _send_request(self, completion_request):
        headers = {
            ...
            'X-CNP-CLOVASTUDIO-API-KEY': self._api_key,
            'X-CNP-APIGW-API-KEY': self._api_key_primary_val,
            'X-CNP-CLOVASTUDIO-REQUEST-ID': self._request_id
        }

        conn = http.client.HTTPSConnection(self._host)
        conn.request('POST', '/testapp/v1/complietions/LK-D', json.dumps(completion_request), headers)
...
preset_input = st.selectbox(
    '사전문장',
    ('MBTI에 대한 지식을 기반으로, 아래 질문에 답해보세요.',
