...
class PensionData():
    def __init__(self, filepath):
        ...
        self.preprocess() # preprocess 함수 호출
    ...
    def preprocess(self):
        ...
        df['사업장명'] = df['사업장명'].apply(self.preprocessing) # preprocessing 함수 매핑
        df['탈퇴일자_연도'] =  pd.to_datetime(df['탈퇴일자']).dt.year
        df['탈퇴일자_월'] =  pd.to_datetime(df['탈퇴일자']).dt.month
        df['시도'] = df['주소'].str.split(' ').str[0]
        df = df.loc[df['가입상태'] == 1].drop(['가입상태', '탈퇴일자'], axis=1).reset_index(drop=True) # 탈퇴한 기업 drop
    ...
    def get_data(self):
        return self.df # 간단히 만들어진 데이터 프레임을 return 해줌
    
@st.cache_data # 아래 데이터 작업(PensionData)을 처음 한번만 수행하고 새로고침 할때마다 다시 작업하지 않도록 해줌. 앱 running 하는 데 굉장히 빨리됨
def read_pensiondata():
    data = PensionData('national-pension.csv')
    return data

data = read_pensiondata()
company_name = st.text_input('회사명을 입력해주세요') # 사용자로부터 회사명을 입력 받음

if data and company_name:
    output = data.find_company(company_name=company_name)
    if len(output) > 0: # 검색이 되었으면 아래 코드 실행
